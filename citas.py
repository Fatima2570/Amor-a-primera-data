...
def calcular_compatibilidad(persona):
    ...
    if altura_buscada == "Misma altura" and abs(altura - persona["Altura"]) <= 5:
        puntos += 1
    elif altura_buscada == "Más alto" and persona["Altura"] > altura:
        puntos += 1
    elif altura_buscada == "Más bajo" and persona["Altura"] < altura:
        puntos += 1
...
if submit:
    if correo in db["Correo"].values:
        st.warning("⚠️ Ya hay una cuenta registrada con este correo. No puedes enviar otra vez el formulario.")
    else:
        nuevo_usuario = {
            "Correo": correo,
            "Nombre": nombre,
        ...
        # Buscar matches compatibles (excluyendo a sí mismo)
        ...
        if not posibles_matches.empty:
            ...
            if not mejores.empty:
                ...
                st.info("Todavía no hay personas que hagan match contigo. ¡Vuelve más tarde a consultar!")
        else:
            st.info("Eres la primera persona en registrarte. Vuelve en un momento para revisar tus matches 😊")